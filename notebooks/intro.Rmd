...
tidy_ceas_sc <- df %>% 
  ...
  mutate(response = as.numeric(str_extract(response, "\\d+$"))) %>% 
  mutate(sub_scale = case_when(
    item %in% c("CEAS-SC2_1", "CEAS-SC2_2", "CEAS-SC2_4", "CEAS-SC2_5") ~ "CEAS_S Action",
    item %in% c("CEAS-SC1_2", "CEAS-SC1_4") ~ "CEAS_S Engagement Dimension 1",
    item %in% c("CEAS-SC1_1", "CEAS-SC1_5", "CEAS-SC1_6", "CEAS-SC1_8") ~ "CEAS_S Engagement Dimension 2",
  ))
...
ceas_sc_scales <- tidy_ceas_sc %>% 
  ...
  filter(!is.na(sub_scale_response)) %>% 
  pivot_wider(ResponseId, names_from = sub_scale, values_from = sub_scale_response) %>% 
  mutate(`CEAS_S Engagement` = `CEAS_S Engagement Dimension 1` + `CEAS_S Engagement Dimension 2`) %>% 
  mutate(CEAS_S = `CEAS_S Engagement` + `CEAS_S Action`)
...
tidy_ceas_to <- df %>% 
  ...
  mutate(response = as.numeric(str_extract(response, "\\d+$"))) %>% 
  mutate(sub_scale = case_when(
    str_detect(item, "CEAS-TO1") ~ "CEAS_TO Engagement",
    str_detect(item, "CEAS-TO2") ~ "CEAS_TO Action"
  ))
...
ceas_to_scales <- tidy_ceas_to %>% 
  ...
  filter(!is.na(sub_scale_response)) %>% 
  pivot_wider(ResponseId, names_from = sub_scale, values_from = sub_scale_response) %>% 
  mutate(CEAS_TO = `CEAS_TO Action` + `CEAS_TO Engagement`)
...
tidy_ceas_from <- df %>% 
  ...
  mutate(sub_scale = case_when(
    str_detect(item, "CEAS-FROM1") ~ "CEAS_FROM Engagement",
    str_detect(item, "CEAS-FROM2") ~ "CEAS_FROM Action"
  ))
...
ceas_from_scales <- tidy_ceas_from %>% 
  ...
  filter(!is.na(sub_scale_response)) %>% 
  pivot_wider(ResponseId, names_from = sub_scale, values_from = sub_scale_response) %>% 
  mutate(CEAS_FROM = `CEAS_FROM Action` + `CEAS_FROM Engagement`)
