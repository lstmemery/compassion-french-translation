...
tidy_scs <- df %>% 
  ...
  mutate(response = as.numeric(str_extract(response, "\\d$"))) %>% 
    mutate(sub_scale = case_when(
    item %in% c("SCS_5", "SCS_12", "SCS_19", "SCS_23", "SCS_26") ~ "CSC_Self_Kindness",
    item %in% c("SCS_1", "SCS_8", "SCS_11", "SCS_16", "SCS_21") ~ "CSC_Self_Judgement",
    item %in% c("SCS_3", "SCS_7", "SCS_10", "SCS_15") ~ "CSC_Common_Humanity",
    item %in% c("SCS_4", "SCS_13", "SCS_18", "SCS_25") ~ "CSC_Isolation",
        item %in% c("SCS_9", "SCS_14", "SCS_17", "SCS_22") ~ "CSC_Mindfulness",
            item %in% c("SCS_2", "SCS_6", "SCS_20", "SCS_24") ~ "CSC_Overidentification",
  ))
...
ceas_sc_scales <- tidy_ceas_sc %>% 
  group_by(ResponseId) %>% 
  summarise(CEAS_SC = sum(response))
...
Need to filter out attention check
...
ceas_to_scales <- tidy_ceas_to %>% 
  group_by(ResponseId) %>% 
  summarise(CEAS_TO = sum(response))
...
tidy_ceas_from <- df %>% 
  ...
  select(ResponseId, starts_with("CEAS-FROM")) %>% 
  pivot_longer(-ResponseId, names_to = "item", values_to = "response") %>%
  mutate(response = as.numeric(str_extract(response, "\\d+$")))
...
ceas_from_scales <- tidy_ceas_from %>% 
  group_by(ResponseId) %>% 
  summarise(CEAS_FROM = sum(response))
