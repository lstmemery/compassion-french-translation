...
```{r}
cor_names <- colnames(clean_cor)[2:ncol(clean_cor)]

results <- map_dfc(cor_names, ~paste0(clean_cor[[.]], p_stars[[.]]))
colnames(results) <- cor_names
results %>% 
  mutate(SOCS = clean_cor[[1]]) %>% 
  select(SOCS, everything())
```
